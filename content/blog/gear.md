---
title: current gear
date: 2025-05-30
tags: [gear, edc, setup]
summary: Current gear and everyday carry setup that I own/have on my person
---

## desk setup 1

- keyboard: [Reddragon Horus TKL-White](https://amzn.in/d/0iZvqJgl)
- mouse1: [Logitech M575S-trackball](https://amzn.in/d/05zGDDvI)
- mouse2: [Logitech MX Master 2S](https://amzn.in/d/05zGDDvI)
- headphones: Moondrop Chu2 &/ dsp
- tablet: Ipad 9th gen w/ [amazon basics pencil](https://amzn.in/d/05zGDDvI)
- speaker: google home mini (got it off of a friend)
- music player: Ipod classic 6th gen w/ [apple earpods](https://www.apple.com/shop/product/mwu53am/a/earpods-35mm-headphone-plug?fnode=1eb7ead777635f655615e9f9e35d1b4aff69d19c6b4d37f4a646b4ddb07c9cc19b2c52dbdbd29e00e1d80b42a8b3070551e9f0aa540f56aa9285858063e61ec646c96386e18f437f0f4ed24329c184436fbee5c1a3ce43477102293907d889a8)
- scratch pad: [Luxor Converge](https://scooboo.in/products/luxor-converge-single-line-pocket-diary)
- mechanical pencil: [Uni kurutoga 0.7mm](https://amzn.in/d/0bfExes3)
- fountain pen: [Delike Moonman Brass](https://amzn.in/d/05zGDDvI)
- light: diy lamp
- desk: custom desk made of mango &/ rosewood
- chair: random office chair
- clock: HMT pilot para shock 17 jewels
- bookstand: a gift by friend from uzbekistan

## computers

- laptop 01: [Rog flow x13 2022(linux, coding &/ primary computer)](https://rog.asus.com/laptops/rog-flow/rog-flow-x13-2022-series/)
- laptop 02: Lenovo ideapad 14ITL6(windows, manager node)
- laptop 03: HP elitebook G1(linux, japanese immersion pc &/ homeserver) ^due for an upgrade^

## edc

- phone: Nothing phone 2a
- ereader: Moded LG Q60
- wallet: custom made leather wallet
- headphones: [oneplus buds 3](https://a.co/d/0fL7sYnF)

## my watches

- watch 01: hmt pilot para shock 17 jewels (mechanical)
- watch 02: titan ns9162sl04 w/ date window
- watch 03: noise vortex
- watch 04: ricoh mechanical (inherited from great grandpa)
- watch 05: maxima sports watch (my first watch)
  ^# some more random watches. I love vintage watches.

---

updated may 30, 2026

---

^# some links if you want to buy - not affiliate

^# I'm probably forgetting some stuff
