...
(function () {
  ...
  /* ─────────── Static manifest (preferred) ─────────────────────────
     deploy.yml runs scripts/build-manifest.py which writes manifest.json
     listing every file under each `dir:` and `photodir:` directive.
     The site reads that manifest first; the GitHub API path below is
     only a fallback for local dev when the manifest hasn't been built.

     Why this exists: the API has a 60-req/hr per-IP rate limit for
     unauthenticated calls. When it returned 403, the blog and notes
     pages went blank for visitors. The manifest removes that dependency
     from the deployed site entirely.                                   */
  async function loadStaticManifest() {
    let res;
    try {
      res = await fetch("manifest.json?v=" + Date.now(), { cache: "no-store" });
    } catch (err) {
      // Network failure — local dev without server, etc. Fall back silently.
      return null;
    }
    if (!res.ok) {
      // 404 is the normal "no manifest yet" case (local dev). Anything else
      // is unexpected — log it so the user sees CI/deploy regressions.
      if (res.status !== 404) console.warn("manifest.json fetch returned", res.status);
      return null;
    }
    try {
      return await res.json();
    } catch (parseErr) {
      console.error("manifest.json is malformed — falling back to GitHub API:", parseErr);
      return null;
    }
  }

  /* ─────────── GitHub directory listing (fallback) ─────────────────
  ...
  async function listDirFromGitHub(dirPath) {
    ...
    try {
      const url = `https://api.github.com/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${encodePath(cleanPath)}?ref=${encodeURIComponent(branch)}`;
  ...
  function setupHero(photos) {
    ...
    function preload(src) { const im = new Image(); im.src = encodePath(src); }
  ...
  function setCrumb(parts) {
    ...
    const segs = list.map((p, i) => {
      ...
      return p.route && !isLast
        ? `<a href="${safeUrl(p.route)}">${display}</a>`
        : `<span>${display}</span>`;
    });
  ...
  function showSubdirs(pageId) {
    ...
    for (const k of kids) {
      ...
      links.push(`<a class="subdir" href="${safeUrl(route)}">[${hashes}${escapeHtml(label)}]</a>`);
    }
  ...
  /* Map an entry to its public hash route (or external URL). External
     URLs from frontmatter (`link:`, `cert:`) are validated via
     safeExternal — anything that isn't http(s)/mailto/tel falls back to
     the internal route so a `link: javascript:...` value can't produce
     a clickable script URL even before HTML escaping. */
  ...
  function postProcessLinks(html) {
    ...
    html = html.replace(/href="#__wiki__\/([^"]+)"/g, (_m, raw) => {
      ...
      if (e) return `href="${safeUrl(routeForEntry(e))}"`;
      const create = newFileUrl(target);
      if (create) {
        return `href="${safeUrl(create)}" target="_blank" rel="noopener noreferrer" class="wikilink-missing" title="Create &ldquo;${escapeHtml(target)}&rdquo; on GitHub"`;
      }
      return `href="#" class="wikilink-missing" title="No entry named &ldquo;${escapeHtml(target)}&rdquo;"`;
    });
    ...
    html = html.replace(/<a([^>]*?)href="([^"]+\.md(?:#[^"]*)?)"([^>]*)>/g, (m, pre, href, post) => {
      ...
      return `<a${pre}href="${safeUrl(route)}"${post}>`;
    });
    return html;
  }
  ...
  function renderContact() {
    ...
    if (links.length) {
      html += `<dl class="contact-grid"${e && e.body ? ' style="margin-top:2rem;"' : ''}>` +
        links.map(l => {
          const href = String(l.href || "");
          const isMail = href.startsWith("mailto:");
          const isExternal = /^https?:/i.test(href);
          const attrs = isExternal ? ' target="_blank" rel="noopener noreferrer"' : '';
          const display = isMail ? href.replace(/^mailto:/, "") : href.replace(/^https?:\/\//i, "");
          return `<dt>${escapeHtml(l.label)}</dt><dd><a href="${safeUrl(href)}"${attrs}>${escapeHtml(display)}</a></dd>`;
        }).join("") +
        `</dl>`;
    }
    $("#page-body").className = "";
    ...
    $("#page-body").innerHTML = groups.map(g => {
      ...
      return `
        <section class="exp-group${isMulti ? ' multi' : ''}">
          ...
          ${g.roles.map(e => {
            const cert = e.cert && !e.cert.includes("REPLACE_ME") && safeExternal(e.cert)
              ? ` <span class="sep">·</span> <a class="cert" href="${safeUrl(e.cert)}" target="_blank" rel="noopener noreferrer">certificate ↗</a>`
  ...
  function renderCertificates() {
    ...
    $("#page-body").innerHTML = list.map(c => {
      const linkPart = c.link && !c.link.includes("REPLACE_ME") && safeExternal(c.link)
        ? `<a href="${safeUrl(c.link)}" target="_blank" rel="noopener noreferrer">view ↗</a>`
  ...
  function renderNotesInline() {
    ...
    if (!list.length) {
      $("#page-body").className = "";
      $("#page-body").innerHTML = `<p class="empty">Drop a new <code>.md</code> file into <code>content/blog/notes/</code> to add a note.</p>`;
      return;
    }
  ...
  function renderList(kind, label, lead) {
    ...
    $("#page-body").className = "list-section";
    $("#page-body").innerHTML = list.map(e => {
      const externalHref = e.link ? safeExternal(e.link) : null;
      ...
      return `
        <article class="list-item" data-slug="${escapeHtml(e.slug)}" data-external="${isExternal ? '1':'0'}" ${isExternal ? `data-href="${escapeHtml(externalHref)}"` : ''}>
          <div class="list-row">
  ...
  function renderReader(slug) {
    ...
    const meta = [];
    meta.push(`<span>${escapeHtml(e.kind)}</span>`);
    if (e.date) meta.push(`<span class="sep">·</span><span>${escapeHtml(fmtDate(e.date))}</span>`);
    if (e.kind === "post" && e.body) meta.push(`<span class="sep">·</span><span>${readingTime(e.body)} min read</span>`);
    if ((e.tags || []).length) meta.push(`<span class="sep">·</span><span>${e.tags.map(t => "#" + escapeHtml(t)).join("  ")}</span>`);
    ...
    $("#page-body").innerHTML = `
      ...
      <p style="margin-top:3rem;font-family:var(--mono);font-size:11px;letter-spacing:.04em;">
        <a href="#/${escapeHtml(section)}">← back to ${escapeHtml(section)}</a>
      </p>
    `;
  }
  ...
  /* Build an attribute-safe URL for an href. Combines scheme validation
     (rejects javascript:, data:, etc.) with HTML escaping. Use this
     everywhere a frontmatter value or anything user-supplied lands inside
     href="${...}". Returns "#" for anything that fails validation. */
  function safeUrl(url) {
    ...
    // Other strings without a scheme are treated as same-origin relative paths.
    if (s && !/^[a-z][a-z0-9+.-]*:/i.test(s)) return escapeHtml(s);
    return "#";
  }

  /* Percent-encode each path segment so filenames with spaces, `#`, `?`,
     or other reserved chars fetch correctly. Preserves the `/` separators. */
  function encodePath(p) {
    return String(p || "").split("/").map(encodeURIComponent).join("/");
  }

  /* Allowlist guard for paths coming out of manifest.json. Prevents a
     poisoned manifest from causing same-origin path traversal or
     protocol-relative cross-origin fetches. */
  function isAllowedPath(p, prefix) {
    if (typeof p !== "string" || !p) return false;
    if (p.includes("..") || p.includes("//")) return false;
    if (/^[a-z][a-z0-9+.-]*:/i.test(p))       return false; // any scheme
    if (p.startsWith("/"))                    return false; // absolute / protocol-relative
  ...
  async function boot() {
    ...
    // Prefer the static manifest.json (built at deploy time). Fall back
    // to the GitHub API only when the manifest is missing (e.g. local
    // dev without running scripts/build-manifest.py first).
    ...
    // files default to `center` and are appended at the end. All
    // manifest paths are validated against an allowlist so a poisoned
    // manifest.json can't fetch arbitrary URLs or traverse out of
    // photos/.
    if ((SITE.photodirs || []).length) {
      const explicitPaths = new Set((SITE.photos || []).map(p => p.src));
      const photoArrays = await Promise.all(SITE.photodirs.map(async d => {
        if (manifest && manifest.photodirs && manifest.photodirs[d]) {
          return manifest.photodirs[d].filter(p => isAllowedPath(p, "photos/"));
        }
        return listPhotosFromGitHub(d);
      }));
      const discovered = photoArrays.flat();
    ...
    // Resolve `dir:` directives → concrete file paths (manifest first).
    const dirFiles = (await Promise.all((SITE.dirs || []).map(async d => {
      if (manifest && manifest.dirs && manifest.dirs[d]) {
        return manifest.dirs[d].filter(p => isAllowedPath(p, "content/"));
      }
      return listDirFromGitHub(d);
    }))).flat();

    // Combine explicit file: lines + auto-discovered dir contents, dedupe.
    // Re-validate `file:` entries too — they're author-supplied but we
    // apply the same allowlist for consistency.
    const seenPaths = new Set();
    const allFiles = [
      ...(SITE.files || []).filter(p => isAllowedPath(p, "content/")),
      ...dirFiles,
    ].filter(p => {
    ...
    // Paths are percent-encoded segment-wise so filenames with spaces,
    // `#`, `?`, etc. fetch as intended instead of being parsed as
    // fragments or query strings.
    const fileResults = await Promise.all(allFiles.map(async (path) => {
      try {
        const r = await fetch(encodePath(path) + "?v=" + Date.now(), { cache: "no-store" });
      ...
      results.innerHTML = matches.map((e, i) => {
        ...
        return `
          <li class="cmdk-result${i === 0 ? " active" : ""}" data-i="${i}">
            <div class="cmdk-row">
              <span class="cmdk-kind">${escapeHtml(kind)}</span>
              <span class="cmdk-title">${escapeHtml(e.title || "(untitled)")}</span>
              <span class="cmdk-date">${escapeHtml(date)}</span>
            </div>
